...
public sealed class PokemonSpeciesDeletionDependencyChecker(PokemonDbContext dbContext)
    : IPokemonSpeciesDeletionDependencyChecker
{
    ...
    {
        var pokemonSpeciesEntityType = dbContext.Model.FindEntityType(typeof(PokemonSpecies))
            ?? throw new InvalidOperationException("Pokemon species entity type is not mapped.");

        var foreignKeys = dbContext.Model
            .GetEntityTypes()
            .SelectMany(static entityType => entityType.GetForeignKeys())
            .Where(foreignKey => foreignKey.PrincipalEntityType == pokemonSpeciesEntityType && !foreignKey.IsOwnership)
            .ToArray();
        ...
        try
        {
            ...
            foreach (var foreignKey in foreignKeys)
            {
                ...
                {
                    blockingReasons.Add(
                        $"Pokemon species cannot be deleted because relation '{foreignKey.DeclaringEntityType.DisplayName()}' cannot be validated automatically.");
                    continue;
                }

                if (await HasReferenceAsync(connection, tableName, schema, columnName, pokemonSpeciesId, cancellationToken))
                {
                    blockingReasons.Add(
                        $"Pokemon species cannot be deleted because it is referenced by '{foreignKey.DeclaringEntityType.DisplayName()}'.");
                }
            }
        ...
        CancellationToken cancellationToken)
    {
        ...
        command.CommandText =
            $"SELECT 1 FROM {QualifyTableName(tableName, schema)} WHERE {QuoteIdentifier(columnName)} = @pokemonSpeciesId LIMIT 1";

        var parameter = command.CreateParameter();
        parameter.ParameterName = "@pokemonSpeciesId";
        parameter.Value = pokemonSpeciesId;
